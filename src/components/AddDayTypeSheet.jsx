...
const COLOR_OPTIONS = ['#5B8DD9',color.sage,'#D4874A',color.honey,color.amberDark,color.rust,color.forest]
...
export default function AddDayTypeSheet({ open, onClose, householdId, onSaved }) {
  ...
  useEffect(() => {
    if (open) { setName(''); setColor(color.sage); setSaving(false) }
  }, [open])
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: color.paper, zIndex: 200, display: 'flex', flexDirection: 'column', maxWidth: '430px', margin: '0 auto' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px', paddingBottom: 120 }}>
        ...
        />
        <div style={{ fontSize: '11px', color: color.inkSoft, fontWeight: 300, marginBottom: '8px' }}>Color</div>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
          {COLOR_OPTIONS.map(c => (
            <button key={c} onClick={() => setColor(c)} style={{
              width: '28px', height: '28px', borderRadius: '50%',
              border: color === c ? `2px solid ${color.ink}` : '2px solid transparent',
        ...
        <button onClick={handleSave} disabled={!name.trim() || saving} style={{
          ...
          background: name.trim() ? color.forest : color.rule, color: name.trim() ? 'white' : color.inkSoft,
