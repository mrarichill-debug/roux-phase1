...
export default function Profile({ appUser }) {
  ...
  const DAY_TYPE_OPTIONS = dayTypeRecords.map(dt => {
    const key = dt.name === 'School Day' ? 'school' : dt.name === 'No School' ? 'no_school' : dt.name.toLowerCase()
    return { key, label: dt.name, color: dt.color || color.inkSoft }
  })
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '22px 24px 14px', animation: 'fadeUp 0.35s ease both' }}>
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '24px', fontWeight: 600, color: color.ink, margin: 0 }}>
          Settings
        </h1>
      </div>

      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px 0', color: color.inkSoft, fontSize: '14px' }}>Loading...</div>
      ) : (
        <>
          {/* ═══ MY ACCOUNT ═══════════════════════════════════════════════ */}
          <div style={{ ...cardStyle, animation: 'fadeUp 0.35s ease 0.04s both' }}>
              ...
              {editingName ? (
                <div style={{ display: 'flex', gap: '6px', flex: 1, alignItems: 'center' }}>
                  <input value={nameValue} onChange={e => setNameValue(e.target.value)} autoFocus
                    ...
                    style={{ flex: 1, padding: '8px 12px', border: `1px solid ${color.sage}`, borderRadius: '8px', fontFamily: "'Jost', sans-serif", fontSize: '14px', color: color.ink, outline: 'none', background: color.paper }} />
                  ...
                  <button onClick={() => setEditingName(false)} style={{ fontSize: '12px', color: color.inkSoft, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Jost', sans-serif" }}>Cancel</button>
                </div>
              ) : (
                <>
                  <div>
                    <div style={{ fontSize: '14px', color: color.ink }}>{displayName}</div>
                    <div style={{ fontSize: '11px', color: color.inkSoft }}>Name</div>
                  </div>
            ...
            <div style={rowStyle}>
              <div>
                <div style={{ fontSize: '14px', color: color.ink }}>{appUser.email}</div>
                <div style={{ fontSize: '11px', color: color.inkSoft }}>Email</div>
              </div>
            </div>
            ...
            <div style={rowStyle}>
              <div>
                <div style={{ fontSize: '14px', color: color.ink }}>••••••••</div>
                <div style={{ fontSize: '11px', color: color.inkSoft }}>Password</div>
              </div>
              <button onClick={sendPasswordReset} style={{ fontSize: '12px', color: resetSent ? color.inkSoft : arcColor, fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Jost', sans-serif" }}>
            ...
            <div style={{ ...rowStyle, flexDirection: 'column', alignItems: 'stretch', gap: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: '14px', color: color.ink }}>Vibration feedback</div>
                  <div style={{ fontSize: '11px', color: color.inkSoft }}>Feel a gentle tap when taking actions</div>
                </div>
            ...
            <div style={{ ...rowStyle, borderBottom: 'none', flexDirection: 'column', alignItems: 'stretch', gap: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: '14px', color: color.ink }}>Weekly planning reminders</div>
                  <div style={{ fontSize: '11px', color: color.inkSoft }}>Sage will nudge you when the week needs attention</div>
                </div>
            ...
            <div style={rowStyle}>
              {editingHome ? (
                <div style={{ display: 'flex', gap: '6px', flex: 1, alignItems: 'center' }}>
                  ...
                  <button onClick={() => { setEditingHome(false); setHomeValue(household.name) }} style={{ fontSize: '12px', color: color.inkSoft, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Jost', sans-serif" }}>Cancel</button>
                </div>
              ) : (
                <>
                  <div>
                    <div style={{ fontSize: '14px', color: color.ink }}>{household?.name}</div>
                    <div style={{ fontSize: '11px', color: color.inkSoft }}>Home name</div>
                  </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{
                      ...
                      color: sel ? s.primary : color.inkSoft,
                      letterSpacing: '0.3px',
                    }}>
                      {SCHEME_NAMES[key]}
                    </span>
            ...
            {members.map(m => {
              ...
              return (
                <div key={m.id} style={rowStyle}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '14px', color: color.ink }}>
                      {m.name}
                      <span style={{ fontSize: isPet ? '13px' : '11px', color: isAdmin ? arcColor : color.inkSoft, fontWeight: isAdmin ? 500 : 400, marginLeft: '8px' }}>
                        {getMemberLabel(m)}
                      </span>
                    </div>
                  </div>
                  {deleteConfirmId === m.id ? (
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      <span style={{ fontSize: '11px', color: color.rust }}>Remove?</span>
                      <button onClick={() => deleteMember(m.id)} style={{ fontSize: '11px', color: 'white', background: color.rust, border: 'none', borderRadius: '6px', padding: '3px 8px', cursor: 'pointer', fontFamily: "'Jost', sans-serif", fontWeight: 500 }}>Yes</button>
                      <button onClick={() => setDeleteConfirmId(null)} style={{ fontSize: '11px', color: color.inkSoft, background: 'none', border: `1px solid ${color.rule}`, borderRadius: '6px', padding: '3px 8px', cursor: 'pointer', fontFamily: "'Jost', sans-serif" }}>No</button>
                    </div>
                  ) : (
            ...
            {stores.map(s => (
              <div key={s.id} style={rowStyle}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '14px', color: color.ink }}>{s.name}</span>
                  {s.is_primary && (
                ...
                {deleteStoreId === s.id ? (
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    <span style={{ fontSize: '11px', color: color.rust }}>Remove?</span>
                    <button onClick={() => deleteStore(s.id)} style={{ fontSize: '11px', color: 'white', background: color.rust, border: 'none', borderRadius: '6px', padding: '3px 8px', cursor: 'pointer', fontFamily: "'Jost', sans-serif", fontWeight: 500 }}>Yes</button>
                    <button onClick={() => setDeleteStoreId(null)} style={{ fontSize: '11px', color: color.inkSoft, background: 'none', border: `1px solid ${color.rule}`, borderRadius: '6px', padding: '3px 8px', cursor: 'pointer', fontFamily: "'Jost', sans-serif" }}>No</button>
                  </div>
                ) : (
          ...
          <div id="tags" style={{ ...cardStyle, animation: 'fadeUp 0.35s ease 0.18s both' }}>
            ...
            {tagDefs.filter(t => t.is_default).map(tag => (
              <div key={tag.id} style={rowStyle}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <svg viewBox="0 0 24 24" fill="none" stroke={color.inkSoft} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 13, height: 13, opacity: 0.5, flexShrink: 0 }}>
                ...
                {editingTagId === tag.id ? (
                  <div style={{ display: 'flex', gap: '6px', flex: 1, alignItems: 'center' }}>
                    <input value={editingTagName} onChange={e => setEditingTagName(e.target.value)} autoFocus
                      ...
                      style={{ flex: 1, padding: '6px 10px', fontSize: '14px', fontFamily: "'Jost', sans-serif", border: `1px solid ${color.sage}`, borderRadius: '8px', outline: 'none', color: color.ink, background: color.paper }} />
                    ...
                    <button onClick={() => setEditingTagId(null)} style={{ fontSize: '12px', color: color.inkSoft, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Jost', sans-serif" }}>Cancel</button>
                  </div>
                ) : deleteTagConfirm === tag.id ? (
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '12px', color: color.ink, marginBottom: '6px' }}>Remove '{tag.name}'? It will be removed from all recipes and meals.</div>
            ...
            {DAY_NAMES.map((day, i) => {
              ...
              return (
                <div key={dowKey} style={{
                  ...
                  <span style={{ fontSize: '13px', color: color.ink }}>{day}</span>
            ...
            <div style={{ marginTop: '14px' }}>
              ...
              {dayTypeRecords.map(dt => (
                <div key={dt.id} style={{
                  ...
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: dt.color || color.inkSoft, flexShrink: 0 }} />
          ...
          <div style={{ padding: '20px 22px 0', textAlign: 'center' }}>
            <div style={{ height: '1px', background: color.rule, marginBottom: '20px' }} />
      ...
      <div style={{ position: 'fixed', inset: 0, background: color.paper, zIndex: 200, display: 'flex', flexDirection: 'column', maxWidth: '430px', margin: '0 auto' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px', paddingBottom: 120 }}>
              ...
              <input type="text" value={newMemberName} onChange={e => setNewMemberName(e.target.value)} placeholder={newMemberIsPet ? "Pet's name" : 'First and last name'} autoFocus style={{
                width: '100%', padding: '12px 14px', border: `1px solid ${color.rule}`, borderRadius: '10px',
                fontFamily: "'Jost', sans-serif", fontSize: '15px', fontWeight: 300, color: color.ink, outline: 'none', background: color.paper, boxSizing: 'border-box', marginBottom: '14px',
              }} />
              ...
              >
                <div style={{
                  ...
                  background: newMemberIsPet ? arcColor : color.rule,
                ...
                <span style={{ fontSize: '13px', color: color.ink, fontWeight: 400 }}>
              ...
              {!newMemberIsPet && (
                <>
                  <div style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '2px', textTransform: 'uppercase', color: color.inkSoft, marginBottom: '6px' }}>Date of birth (optional)</div>
                  <input type="date" value={newMemberDob} onChange={e => setNewMemberDob(e.target.value)} style={{
                    width: '100%', padding: '12px 14px', border: `1px solid ${color.rule}`, borderRadius: '10px',
                    fontFamily: "'Jost', sans-serif", fontSize: '15px', fontWeight: 300, color: color.ink, outline: 'none', background: color.paper, boxSizing: 'border-box', marginBottom: '6px',
                  }} />
                  <div style={{ fontSize: '11px', fontStyle: 'italic', color: color.inkSoft, marginBottom: '14px', lineHeight: 1.4 }}>
              ...
              {!newMemberIsPet && (
                <>
              <div style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '2px', textTransform: 'uppercase', color: color.inkSoft, marginBottom: '6px' }}>Role</div>
                ...
                <div style={{ display: 'flex', gap: '6px', marginBottom: '14px' }}>
                  {[
                    ...
                    <button key={r.key} onClick={() => setNewMemberRole(r.key)} style={{
                      ...
                      border: `1.5px solid ${newMemberRole === r.key ? arcColor : color.rule}`,
                      background: newMemberRole === r.key ? arcColor : 'transparent',
                      color: newMemberRole === r.key ? 'white' : color.ink, transition: 'all 0.15s',
                      textAlign: 'center',
                    }}>
                      {r.label}
                    </button>
              ...
              <button onClick={editMemberId ? saveMemberEdit : addMember} disabled={!newMemberName.trim() || savingMember} style={{
                width: '100%', padding: '14px', borderRadius: '12px', background: newMemberName.trim() ? arcColor : color.rule,
                color: newMemberName.trim() ? 'white' : color.inkSoft, border: 'none', fontFamily: "'Jost', sans-serif",
      ...
      <div style={{ position: 'fixed', inset: 0, background: color.paper, zIndex: 200, display: 'flex', flexDirection: 'column', maxWidth: '430px', margin: '0 auto' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px', paddingBottom: 120 }}>
              ...
              <button onClick={addStore} disabled={!newStoreName.trim()} style={{
                width: '100%', padding: '14px', borderRadius: '12px', background: newStoreName.trim() ? arcColor : color.rule,
                color: newStoreName.trim() ? 'white' : color.inkSoft, border: 'none', fontFamily: "'Jost', sans-serif",
      ...
      <BottomSheet isOpen={transferOpen} onClose={() => setTransferOpen(false)} zIndex={300} title="Transfer Admin">
            <div style={{ padding: '0 22px 40px' }}>
              <div style={{ fontSize: '13px', color: color.inkSoft, lineHeight: 1.5, marginBottom: '16px' }}>
                Select a Co-admin to become the new Admin. You will become Co-admin.
              </div>
              {members.filter(m => getMemberRole(m) === 'Co-admin').length === 0 ? (
                <div style={{ fontSize: '13px', fontStyle: 'italic', color: color.inkSoft, padding: '16px 0' }}>
                ...
                members.filter(m => getMemberRole(m) === 'Co-admin').map(m => (
                  <button key={m.id} onClick={() => {
                    ...
                    border: '1px solid rgba(200,185,160,0.55)', background: color.paper,
                    cursor: 'pointer', fontFamily: "'Jost', sans-serif", fontSize: '14px', color: color.ink,
                    textAlign: 'left',
                  }}>
                    <span>{m.name}</span>
...
function ToggleSwitch({ on, onToggle }) {
  ...
  return (
    <button onClick={onToggle} style={{
      ...
      border: on ? 'none' : `1.5px solid ${color.rule}`,
      background: on ? arcColor : color.paper,
