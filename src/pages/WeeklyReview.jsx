...
export default function WeeklyReview({ appUser }) {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ background: C.forest, padding: '20px 22px 16px' }}>
      ...
      <div style={{ padding: '16px 22px' }}>
        {meals.map(meal => {
          ...
          return (
            <div key={meal.id} style={{
              background: 'white', borderRadius: '14px', border: `1px solid ${isReviewed ? 'rgba(122,140,110,0.3)' : C.linen}`,
              ...
              <div onClick={() => mode === 'detailed' && setExpandedMeal(isExpanded ? null : meal.id)} style={{
                ...
                <div>
                  <div style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '0.8px', textTransform: 'uppercase', color: C.driftwood, marginBottom: '2px' }}>
                    {getMealDay(meal)}
                  </div>
                  <div style={{ fontSize: '15px', color: isCooked ? C.sage : C.ink, fontWeight: 400 }}>
                    {isCooked && <span style={{ color: C.sage }}>✓ </span>}
                    {isSkipped && <span style={{ color: C.driftwood }}>— </span>}
                    {meal.displayName}
                  </div>
                </div>
                {isReviewed && !isExpanded && (
                  <span style={{ fontSize: '10px', color: isCooked ? C.sage : C.driftwood, fontWeight: 500 }}>
              ...
              {isExpanded && isReviewed && (
                <div style={{ marginTop: '12px', borderTop: `1px solid ${C.linen}`, paddingTop: '12px' }}>
                  {isCooked && (
                    <div style={{ marginBottom: '12px' }}>
                      <div style={{ fontSize: '11px', color: C.driftwood, marginBottom: '6px' }}>Did you use all the ingredients?</div>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        {['yes', 'mostly', 'no'].map(val => (
                          <button key={val} onClick={() => setDetailedField(meal.id, 'ingredients_consumed', val)} style={{
                            ...
                            border: meal.ingredients_consumed === val ? `1.5px solid ${C.forest}` : `1px solid ${C.linen}`,
                            background: meal.ingredients_consumed === val ? 'rgba(61,107,79,0.08)' : 'white',
                            color: meal.ingredients_consumed === val ? C.forest : C.ink,
                  ...
                  {isSkipped && (
                    <div style={{ marginBottom: '12px' }}>
                      <div style={{ fontSize: '11px', color: C.driftwood, marginBottom: '6px' }}>Did you buy ingredients before skipping?</div>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        {[true, false].map(val => (
                          <button key={String(val)} onClick={() => setDetailedField(meal.id, 'ingredients_bought_before_skip', val)} style={{
                            flex: 1, padding: '8px', borderRadius: '8px', fontSize: '12px',
                            border: meal.ingredients_bought_before_skip === val ? `1.5px solid ${C.forest}` : `1px solid ${C.linen}`,
                            background: meal.ingredients_bought_before_skip === val ? 'rgba(61,107,79,0.08)' : 'white',
                            color: meal.ingredients_bought_before_skip === val ? C.forest : C.ink,
                  ...
                  {/* Review rating */}
                  <div style={{ marginBottom: '12px' }}>
                    <div style={{ fontSize: '11px', color: C.driftwood, marginBottom: '6px' }}>Would you make this again?</div>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      {[{ val: 'hit', label: 'Hit', emoji: '👍' }, { val: 'skip', label: 'Skip', emoji: '👎' }, { val: 'maybe', label: 'Maybe', emoji: '🤔' }].map(opt => (
                        <button key={opt.val} onClick={() => setDetailedField(meal.id, 'review_rating', opt.val)} style={{
                          flex: 1, padding: '8px', borderRadius: '8px', fontSize: '12px',
                          border: meal.review_rating === opt.val ? `1.5px solid ${C.forest}` : `1px solid ${C.linen}`,
                          background: meal.review_rating === opt.val ? 'rgba(61,107,79,0.08)' : 'white',
                          color: meal.review_rating === opt.val ? C.forest : C.ink,
                          cursor: 'pointer', fontFamily: "'Jost', sans-serif",
        ...
        <div style={{ textAlign: 'center', marginTop: '20px' }}>
          <button onClick={() => setMode(mode === 'quick' ? 'detailed' : 'quick')} style={{
            ...
            fontSize: '12px', color: C.driftwood, fontFamily: "'Jost', sans-serif", fontWeight: 300, fontStyle: 'italic',
