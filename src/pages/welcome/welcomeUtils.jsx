...
export function MiniWordmark() {
  return (
    <span style={{
      ...
      color: color.forest, letterSpacing: '-0.5px',
    }}>
      Roux.
    </span>
  )
...
export function BackButton({ onClick }) {
  return (
    <button
      ...
      onMouseDown={e => e.currentTarget.style.background = color.rule}
      onMouseUp={e => e.currentTarget.style.background = 'white'}
      onTouchStart={e => e.currentTarget.style.background = color.rule}
...
export function ProgressBar({ step, total }) {
  ...
  return (
    <div style={{ padding: '24px 24px 0', position: 'relative', zIndex: 1, opacity: 0, animation: 'fadeIn 0.4s ease 0.1s forwards' }}>
      <div style={{ height: '3px', background: color.rule, borderRadius: '2px', overflow: 'hidden', marginBottom: '8px' }}>
        <div style={{
          height: '100%', background: color.forest, borderRadius: '2px',
          width: `${pct}%`, transition: 'width 0.4s cubic-bezier(0.22,1,0.36,1)',
        }} />
      </div>
      <div style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '2px', textTransform: 'uppercase', color: color.inkSoft }}>
        {label}
      </div>
    </div>
  )
  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? color.rule : color.forest,
        color: disabled ? color.inkSoft : 'white',
      ...
      onMouseDown={e => !disabled && !loading && (e.currentTarget.style.background = color.forestDark)}
      onMouseUp={e => !disabled && !loading && (e.currentTarget.style.background = color.forest)}
      onTouchStart={e => !disabled && !loading && (e.currentTarget.style.background = color.forestDark)}
      onTouchEnd={e => !disabled && !loading && (e.currentTarget.style.background = color.forest)}
    >
      {loading ? (
        <div style={{
...
export function PasswordStrength({ password, visible }) {
  ...
  const barColor = score === 1 ? color.honey : score === 2 ? color.sage : score === 3 ? color.forest : color.rule
    ...
    <div style={{ marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
      <div style={{ display: 'flex', gap: '4px' }}>
        {[1, 2, 3].map(i => (
          <div key={i} style={{
            ...
            background: i <= score ? barColor : color.rule,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '6px', transition: 'color 0.2s', color: met ? color.forest : color.inkSoft, fontSize: '11px', fontWeight: 300 }}>
            <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: met ? color.forest : color.rule, flexShrink: 0, transition: 'background 0.2s' }} />
            {label}
          </div>
        ))}
      </div>
...
export function FormField({ label, error, hint, children }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
      <div style={{ fontSize: '11px', fontWeight: 500, letterSpacing: '1.5px', textTransform: 'uppercase', color: color.inkSoft, paddingLeft: '2px' }}>
        {label}
      </div>
      {children}
      {hint && !error && <div style={{ fontSize: '11.5px', color: color.inkSoft, fontWeight: 300, paddingLeft: '2px', marginTop: '2px', lineHeight: 1.5 }}>{hint}</div>}
      {error && <div style={{ fontSize: '11.5px', color: color.rust, fontWeight: 300, paddingLeft: '2px', marginTop: '2px' }}>{error}</div>}
    </div>
  )
}
...
export function inputStyle({ focused, hasValue, hasError }) {
  let border = `1.5px solid ${color.rule}`
  let boxShadow = 'none'
  if (hasError) { border = `1.5px solid ${color.rust}`; boxShadow = '0 0 0 3px rgba(160,48,48,0.08)' }
  else if (focused) { border = `1.5px solid ${color.sage}`; boxShadow = '0 0 0 3px rgba(122,140,110,0.12)' }
  ...
  return {
    ...
    fontFamily: "'Jost', sans-serif", fontSize: '15px', fontWeight: 300, color: color.ink,
...
export function TosLine() {
  return (
    <p style={{ fontSize: '10px', color: color.inkSoft, textAlign: 'center', lineHeight: 1.7, marginTop: '12px', fontWeight: 300 }}>
...
export function StepHead({ eyebrow, title, sub, animDelay = '0.15s' }) {
  return (
    <div style={{ padding: '32px 0 28px', opacity: 0, animation: `fadeUp 0.45s ease ${animDelay} forwards` }}>
      {eyebrow && (
        <div style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '2.5px', textTransform: 'uppercase', color: color.sage, marginBottom: '8px' }}>
          {eyebrow}
        </div>
      )}
      <div style={{ fontFamily: "'Playfair Display', serif", fontSize: '26px', fontWeight: 500, color: color.ink, lineHeight: 1.25, marginBottom: '8px' }}>
        {title}
      </div>
      {sub && (
